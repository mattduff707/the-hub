...
const createSortedList = (arr) => {
  const favoritesList = arr
    .filter((bookmark) => bookmark.favorite)
    .sort(sortBookmarks);
  const othersList = arr
    .filter((bookmark) => !bookmark.favorite)
    .sort(sortBookmarks);
...
const reducer = (
  state = { loading: true, error: false, bookmarks: [] },
  action
) => {
  switch (action.type) {
    case "INIT_BOOKMARKS":
    ...
    case "REMOVE_BOOKMARK":
      const stateWithBookmarkRemoved = state.bookmarks.filter(
        (bookmark) => bookmark._id !== action.payload
      );
      return { ...state, bookmarks: stateWithBookmarkRemoved };
    case "EDIT_BOOKMARK":
